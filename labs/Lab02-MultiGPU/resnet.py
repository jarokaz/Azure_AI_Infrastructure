...
def model(img_input):
    ...
    x = conv_block(x, 3, [64, 64, 64], stage=1, block='a', s=2)
    ...
    x = conv_block(x, 3, [128, 128, 128], stage=2, block='a', s=2)
    ...
    x = conv_block(x, 3, [256, 256, 256], stage=3, block='a', s=2)
    ...
    x = conv_block(x, 3, [512, 512, 512], stage=4, block='a', s=2)
    x = identity_block(x, 3, [512, 512, 512], stage=3, block='b')
    x = identity_block(x, 3, [512, 512, 512], stage=3, block='c')
    x = identity_block(x, 3, [512, 512, 512], stage=3, block='d')
    x = identity_block(x, 3, [512, 512, 512], stage=3, block='e')

    x = MaxPooling2D(pool_size=2, name='avg_pool')(x)
